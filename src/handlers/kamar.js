...
export async function createKamar(env, request) {
	try {
		...
		const stmt = env.DB.prepare('INSERT INTO kamar (nama, lokasi, harga) VALUES (?, ?, ?)').bind(nama, lokasi, harga);
...
export async function updateKamar(env, request, id) {
	...
	try {
		...
		const stmt = env.DB.prepare('UPDATE kamar SET nama = ?, lokasi = ?, harga = ? WHERE id = ?').bind(nama, lokasi, harga, id);
